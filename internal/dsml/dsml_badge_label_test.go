...
// The label is not a DSML tag name, so the known-name junk sweeper never
// touched it. normalizeDSMLText must rewrite the tail-anchored, pipe-marked
// residue to the canonical close so the gate and parser see it.
func TestIsDSMLToolCallEndBadgeLabelClose(t *testing.T) {
	lt, gt := "<", ">"
	ff := string(rune(0xFF5C))
	// The non-breaking hyphen (U+2011) is from the real capture, but the rule
	// does not special-case it: the regex discards the whole label ([^<>]*),
	// so a regular hyphen works identically.
