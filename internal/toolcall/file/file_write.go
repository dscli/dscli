...
func handleWriteFile(ctx context.Context, args toolcall.ToolArgs) (result string, user string, err error) {
	...
	if apath == "" || content == "" {
		if truncated {
			user = fmt.Sprintf(`内容超过 max_tokens=8192, 已经截断。其中apath参数也被截断,
因此截断内容虽然收到但因不知写入哪个文件，最终无法写入。
请严格按照 write_file 的使用说明，严控输出长度不超 8192 Tokens，
在输出内容(content)前，优先考虑输出到哪个文件（apath 参数），
注意content无法写入已丢弃，请重新生成这部分content，
以下将已输出content内容的最后几行也附上供参考:

%s`, lastlines)
		}
		err = fmt.Errorf("文件路径 apath 或文件内容 content 不能为空")
		return
	}
	...
	if append {
		...
		if err != nil {
			err = fmt.Errorf("无法打开文件进行追加: %w", err)
			return
		}
		defer file.Close()
		...
		outfmt.Notice("追加内容到文件 \"%s\"，添加 %d 行", apath, lines)

		// 构建最终结果
		result = fmt.Sprintf("成功追加内容到文件 \"%s\"，添加 %d 行。", apath, lines)
	} else {
		// 非追加模式：使用现有的行范围写入
		result, _, err = handleWriteFileWithLineRange(ctx, args)
	}

	if truncated {
		user = fmt.Sprintf(`此次写入文件 %s 的内容是截断的内容，其中最后一行因为不完整已丢弃，
请从上次输出内容的最后一完整行继续生成，并调用工具 write_file(apath="%s", append=true, content="...继续生成的内容...") 
追加入文件%s，为帮助你找到继续生成的点，现把上次截断内容最后几行展示给你：
---
%s---
如果觉得信息不足以继续生成，可以停下来询问。`, apath, apath, apath, lastlines)
	} else {
