...
func init() {
	toolcall.RegisterTool(toolcall.ToolDef{
		...
		Strict:      true,
		// read_file_with_line_range 是旧名:2026-08 合并四个文件工具为两个 (read_file/write_file),
		// 注册为别名让历史会话中的旧调用名继续可用 (GetToolDef 解析,工具列表不显示)。
		Aliases: []string{"read_file_with_line_range"},
...
// handleReadFile 读取文件完整内容或指定行范围。
// 输出格式为 NR:TAG content，其中 TAG 是 4 字符校验和(CAS)标签。
// TAG 可传递给 write_file 的 line_tag/line_tags 参数防止竞态写入。
