...
func LoadSkills(ctx context.Context) (messages []Message, err error) {
	...
	builder.WriteString("2. Extract the script from the returned content\n")
	builder.WriteString("3. Execute the script via `shell` or `python` tool\n\n")
	...
	builder.WriteString("Step 2: Find the script block (e.g. starting with #!/usr/bin/env bash)\n")
	builder.WriteString("Step 3: Execute via shell tool with that script content\n")
	builder.WriteString("```\n")
