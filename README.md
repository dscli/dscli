...
- v0.9.3 (2026-09-01) — **Architect role & quality gates**: new built-in **architect** role (software architect persona: clarifies requirements, designs architecture, orchestrates `code_dev` / `code_review` / `quality_assurance` pipeline); `chat --role` now defaults to architect, webchat `--role` gains architect; DSML tool set follows per-role tool config — `apply_patch` removed (31% success rate), legacy `exec_command` mapping confined to the DSML layer, four file tools merged into `read_file`/`write_file` (line-range edits, CAS tags, legacy append / lone tag rejected); `keeprunning` prevents idle screen lock during chat/webchat; DSML parser hardened against chat.deepseek.com badge-label variants, slash-less `<tool_calls>` close, truncated/duplicated-wrapper samples and malformed emissions (re-issued with warning, never executed); role config cache scoped per session with lost-invalidation guard; webchat `keep` resume skips role prompt injection; review role gains cyclomatic-complexity gate (gocyclo over 20 → findings), QA role gains coverage measurement step (`go test -coverprofile`, zero-coverage core paths affect verdict); Makefile `fmt-check` fixed (fails on diffs/tool errors), em-dash cleanup
