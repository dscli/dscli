...
| `internal/lp/` | Web page reading via `lightpanda fetch` CLI (DeepSeek share links read the full conversation from the share-content API: the page's virtualized message list truncates DOM dumps; see `share.go`), DeepSeek web login/chat (chromedp) with overload/truncation detection and conversation registry; `HandleWebChat` is the high-level entry point (retry + DSML tool loop) shared by ask_expert and the webchat CLI; `SafeUploadName` normalizes attachment names to the extensions the site accepts (2026-09-13 live probe: an extension outside the verified set, or no extension at all, is uploaded under the same name with `.txt` appended); a server-stopped generation (「继续生成」 button) is auto-resumed with a real CDP mouse click because that button is the one element whose onClick validates `isTrusted` and silently drops synthetic clicks (see `docs/task-continue-generation.md`); a server-stopped round with NO output at all (the 「已停止」 header, no 「继续生成」 button) is instead auto-regenerated by clicking the action bar's second icon button through the same real-CDP path (see `docs/task-stop-recovery-and-residue.md`) |
| `internal/dsml/` | DSML tool-call protocol for WebChat: parse and judge replies (`ParseDSMLMessage`), render the role's tool doc (`BuildDSMLToolDoc`), execute parsed calls (`ExecuteDSMLToolCalls`); no separate whitelist - the role's `role_configs` tool set gates execution. Exports `CodeRanges`/`InRanges` (quoted-content scanner), `MarkerRanges` (DSML marker-shape locator used by the shell channel's residue warning), and `BlockedCmdRe` (destructive-command policy) for `internal/shellblock` |
| `internal/shellblock/` | `<shell>` block protocol for WebChat (the dev role / code_dev channel): judge + warnings (`Judge`, `ShouldEnter`), destructive-command interception (`Blocked`, shared with dsml), local runner (`Run`: scriptN.sh / scriptN.txt, process-group timeout, 256KB output cap), tool doc builder (`BuildToolDoc`); its judge computes `Verdict.ResidualMarkers` (DSML marker shapes outside the executed block and quoted code) and the shell loop rides `ResidueNote()` along with the round feedback; see `docs/task-shell-block.md` and `docs/task-stop-recovery-and-residue.md` |
