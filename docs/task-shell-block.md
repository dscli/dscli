...
- 执行前拦截（安全边界，不许降级）：整段脚本先过 `dsml.BlockedCmdRe`（自 `dsmlBlockedCmdRe` 导出，名单一字不动）；命中 → 本轮不写文件、不执行，回填拦截拒收警告（要求改写重发；计入连续告警）。依据 scripts.txt L2965：这一层不能省，且要比 DSML 更严——实现取 fail-closed。
...
- 包测试：判定表全分支；提取边界（徽章残形 / 重复标签 / 乱序 / 围栏 / heredoc / `<script.sh>` 字面量）；引用区域剔除（围栏内块不执行 / 围栏示例后跟真块 / 行内代码 / 未闭合围栏吞块）；拦截拒收（不执行、不落文件）；告警文案（含 DSML 形状追加句）；runner 超时 / 退出码 / 合并输出。
