...
func TestRetryDelayCalculation(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			name:     "第一次重试",
			attempt:  1,
			expected: 10 * time.Second, // 2^0 * 10s = 10s
		},
		{
			name:     "第二次重试",
			attempt:  2,
			expected: 20 * time.Second, // 2^1 * 10s = 20s
		},
		{
			name:     "第三次重试",
			attempt:  3,
			expected: 40 * time.Second, // 2^2 * 10s = 40s
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			delay := min(time.Duration(1<<(tc.attempt-1))*client.retryDelay,
				300*time.Second)
