...
4. **推送远程流程**：
   - 经过专家审阅并修复问题后，及时推送到远程仓库
   - 保持远程仓库与本地同步
   - 遵循"凡是有专家review过的代码都可以push"的原则

## 新工具：系统提示词管理
- **system_prompt**：获取当前系统提示词，帮助理解工作环境和约束条件
  - 支持多种格式：完整内容、简要摘要、原始模板、模板变量
  - 使用场景：了解当前权限、调试问题、学习最佳实践
